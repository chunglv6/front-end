...
export class RepresentativeComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes) {
    if (changes.setValueForm.currentValue.data?.listData.length > 0) {
      ...
      this.getListDocumentType(changes.setValueForm.currentValue.data.listData[0].repIdentityTypeId);
    }
  }
  ...
  setDisable() {
    this.customerEnterpriseForm.controls.documentTypeName.disable();
  ...
  patchValueForm(formValue) {
    if (formValue) {
      this.customerEnterpriseForm.patchValue({
        fullName: formValue.repName,
        dateOfBirth: moment(formValue.repBirthDate, COMMOM_CONFIG.DATE_FORMAT).toDate(),
        gender: formValue.repGender,
        documentNumber: formValue.repIdentityNumber,
        dateRange: moment(formValue.repDateOfIssue, COMMOM_CONFIG.DATE_FORMAT).toDate(),
        placeOfIssue: formValue.repPlaceOfIssue,
        street: formValue.repStreet,
        address: formValue.repAreaName,
        numberPhone: formValue.repPhoneNumber,
        email: formValue.repEmail
      });
    }
  }
  ...
  getListDocumentType(valueSelected) {
    this._sharedDirectoryService.getListDocumentType().subscribe(res => {
      ...
      const findDocType = this.listOptionDocumentType.find(x => x.code == valueSelected);
      if (findDocType) {
        this.customerEnterpriseForm.get('documentType').patchValue(findDocType.code);
        this.customerEnterpriseForm.get('documentTypeName').patchValue(findDocType.value);
      }
