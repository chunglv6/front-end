...
export class PriorityForbiddenVehicleComponent extends BaseComponent implements OnInit, AfterViewChecked, OnDestroy {
  ...
  constructor(protected toars: ToastrService,
    ...
    super(actr, vehicleService, RESOURCE.EXCEPTION, toars, translateService, dialog);
  }
  ngOnDestroy(): void {
    this.onDestroy.next();
  ...
  buildForm() {
    this.formSearch = this.fb.group({
      ...
      cancellationDateFrom: null, // Ngày hủy: Từ ngày - đến ngày
  ...
  getListPriorityType() {
    this._sharedDirectoryService.getCategories('WHITELIST_TYPE').subscribe(rs => {
      this.listOptionPriorityType = rs.data.listData.map(val => {
        return {
          id: val.id,
          code: val.code,
          value: val.name
      ...
      if (!AppStorage.get('white-list-type-vehicle')) {
        AppStorage.set('white-list-type-vehicle', this.listOptionPriorityType);
      }
    });
  }
      ...
      if (dialogResult) {
        this.vehicleService.deleteVehicleException(vehicle.exceptionListId).subscribe(rs => {
          if (rs.mess.code === HTTP_CODE.SUCCESS) {
            ...
            this.toars.warning(this.translateService.instant('special-vehicle.vehicleCanNotDelete'));
          }
        }, err => {
        ...
        this.vehicleService.approveVehicleException(data).subscribe(res => {
          ...
          if (res.mess.code === HTTP_CODE.SUCCESS) {
            ...
            if (res.data.listWarning.length > 0) {
              this.toars.warning(res.data.messWarning);
            }
  ...
  convertData(isExport?) {
    ...
    if (this.searchModel.createDateFrom) {
      this.searchModel.createDateFrom = moment(this.searchModel.createDateFrom).format(COMMOM_CONFIG.DATE_FORMAT);
    }
    if (this.searchModel.createDateTo) {
      this.searchModel.createDateTo = moment(this.searchModel.createDateTo).format(COMMOM_CONFIG.DATE_FORMAT);
    }
    if (this.searchModel.approvalDateFrom) {
      this.searchModel.approvalDateFrom = moment(this.searchModel.approvalDateFrom).format(COMMOM_CONFIG.DATE_FORMAT);
    }
    if (this.searchModel.approvalDateTo) {
      this.searchModel.approvalDateTo = moment(this.searchModel.approvalDateTo).format(COMMOM_CONFIG.DATE_FORMAT);
    }
    if (this.searchModel.effectiveDateFrom) {
      this.searchModel.effDateFrom = moment(this.searchModel.effectiveDateFrom).format(COMMOM_CONFIG.DATE_FORMAT);
      delete this.searchModel.effectiveDateFrom;
    }
    if (this.searchModel.effectiveDateTo) {
      this.searchModel.effDateTo = moment(this.searchModel.effectiveDateTo).format(COMMOM_CONFIG.DATE_FORMAT);
      delete this.searchModel.effectiveDateTo;
    }
    if (this.searchModel.cancellationDateFrom) {
      this.searchModel.cancelDateFrom = moment(this.searchModel.cancellationDateFrom).format(COMMOM_CONFIG.DATE_FORMAT);
      delete this.searchModel.cancellationDateFrom;
    }
    if (this.searchModel.cancellationDateTo) {
      this.searchModel.cancelDateTo = moment(this.searchModel.cancellationDateTo).format(COMMOM_CONFIG.DATE_FORMAT);
