...
def deploy_module_web(server,groupId,artifactId){
    ...
    sh """
        pwd
        ansible-playbook cicd/deploy/deploy_etc_crm-webapp_ui_${server}.yml -e http_app_port=${http_app_port} -e groupId=${groupId} -e artifactId=${artifactId} -e BUILD_NUMBER=${BUILD_NUMBER}
    """
}
...
def buildPushCommit() {
    ...
    def crmFrontend = readJSON file: 'package.json'
    def crmFrontendVersion = crmFrontend.version
    def version = "${crmFrontendVersion}_${env.gitlabBranch}_u${BUILD_NUMBER}"
    ...
    tasks['Package and Build Artifact'] = {
        node("$env.node_slave") {
            buildService("PUSH","${version}","crm-frontend")
        }
    }
    parallel tasks
    def uploads = [:]
    ...
    if(env.gitlabBranch == env.STAGING_BRANCH){
        uploads['Push Image To Repo Docker'] = {
            node("$env.node_slave"){
                stage('Push Image To Repo Docker'){
                    pushImageToDockerRepo("crm-frontend","${version}")
                }
            }
        }
        parallel uploads
        deploys['Deploy to K8s'] = {
            node("$env.node_slave"){
               stage('Deploy to K8s'){
                    echo 'deploy to K8s'
                    release2k8s("${version}","etc-staging","crm-frontend","config_108_172")
                }
            }
        }
        parallel deploys
        ...
        parallel uploads
         deploys['Deploy to K8s TestLab'] = {
            node("$env.node_slave"){
               stage('Deploy to K8s'){
                    echo 'deploy to K8s'
                    release2k8s("${version}","etc","crm-frontend","config_155_31")
                }
            }
        }
        parallel deploys
...
def buildMergeRequest() {
    ...
    def crmFrontend = readJSON file: 'package.json'
    def crmFrontendVersion = crmFrontend.version
    def version = "${crmFrontendVersion}_${env.gitlabTargetBranch}_u${BUILD_NUMBER}"
    env.project_version=version
    def tasks = [:]
    ...
    tasks['Package and Build Artifact'] = {
        node("$env.node_slave") {
            buildService("MERGE","${version}","crm-frontend")
        }
    }
    parallel tasks
    def uploads = [:]
    ...
    if(env.gitlabTargetBranch == env.STAGING_BRANCH){
        uploads['Push Image To Repo Docker'] = {
            node("$env.node_slave"){
                stage('Push Image To Repo Docker'){
                    pushImageToDockerRepo("crm-frontend","${version}","crm-frontend")
                }
            }
        }
        parallel uploads
